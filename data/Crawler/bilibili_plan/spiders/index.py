...
class VideoSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        cv_raw[0] =cv_raw[0][1:-1] + '\\n'
        cv_list = clean_array1(re.findall('(.*?)n', str(cv_raw)[2:]))
        bangumi_item["cv"] = clean_string(divider.join(cv_list).replace('\\', '').replace('、', ',').replace('u002F', ',').replace('u200b', ''))
        staff_raw = re.findall('"staff":"(.*?)","stat"', response.text)
        staff_raw[0] = staff_raw[0]+"n"
        staff_list = clean_array(re.findall('(.*?)n', str(staff_raw).replace('u002F','/').replace("['",'').replace('\\','')))
        bangumi_item["staff"] = clean_string(divider.join(staff_list).replace('、', '/').replace('u200b', ''))
        yield bangumi_item
...
def clean_array1(arr):
    for i in range(len(arr)):
        arr[i] = clean_item1(arr[i])
    return arr
